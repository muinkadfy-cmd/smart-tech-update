...
contextBridge.exposeInMainWorld('electron', {
  ...
  update: {
    ...
    checkOnlineRailway: () => ipcRenderer.invoke('update-check-online-railway'),
    // Baixar atualização (Railway)
    downloadRailway: (downloadUrl) => ipcRenderer.invoke('update-download-railway', downloadUrl),
    ...
    autoUpdater: {
      ...
      onUpdateAvailable: (callback) => {
        ipcRenderer.on('auto-updater-update-available', (_, data) => callback(data));
        return () => ipcRenderer.removeAllListeners('auto-updater-update-available');
      },
      onUpdateNotAvailable: (callback) => {
        ipcRenderer.on('auto-updater-update-not-available', (_, data) => callback(data));
        return () => ipcRenderer.removeAllListeners('auto-updater-update-not-available');
      },
      onDownloadProgress: (callback) => {
        ipcRenderer.on('auto-updater-download-progress', (_, data) => callback(data));
        return () => ipcRenderer.removeAllListeners('auto-updater-download-progress');
      },
      onUpdateDownloaded: (callback) => {
        ipcRenderer.on('auto-updater-update-downloaded', (_, data) => callback(data));
        return () => ipcRenderer.removeAllListeners('auto-updater-update-downloaded');
      },
      onError: (callback) => {
        ipcRenderer.on('auto-updater-error', (_, data) => callback(data));
        return () => ipcRenderer.removeAllListeners('auto-updater-error');
      }
    },
